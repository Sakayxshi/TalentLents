...
export default function TeamEfficiencyPage() {
  ...
  const { employees, roster, upskillCandidates, externalCandidates, shortlistedCandidates, scenarios, selectedScenarioId, markPageComplete } = useStore();
  ...
  const dimensions = useMemo(() => {
    ...
    const coveredSkills = uniqueReqSkills.filter(s =>
      rosterEmps.some(e => (e.technical_skills || '').toLowerCase().includes(s.toLowerCase()))
    );
    ...
    const coveredCerts = uniqueReqCerts.filter(c =>
      rosterEmps.some(e => (e.certifications || '').toLowerCase().includes(c.toLowerCase()))
    );
    ...
    // Single points of failure: skills held by only 1 person
    const skillHolders: Record<string, number> = {};
    uniqueReqSkills.forEach(s => {
      const count = rosterEmps.filter(e => (e.technical_skills || '').toLowerCase().includes(s.toLowerCase())).length;
      if (count === 1) skillHolders[s] = 1;
    });
  ...
  const warnings = useMemo(() => {
    ...
    scenario.roles.forEach(r => {
      ...
      if (filled < r.headcount * 0.5) {
        warns.push({ type: 'critical', text: `${r.role} has only ${Math.round((filled / Math.max(r.headcount, 1)) * 100)}% coverage — ${r.headcount - filled} positions remain unfilled`, link: '/gap-analysis' });
      }
    });

    if (highRisk.length > 0) {
      warns.push({ type: 'warning', text: `${highRisk.length} high flight-risk employees on roster — retention actions recommended`, link: '/workforce' });
    }
    ...
    if (wellStaffed.length > 0) {
      warns.push({ type: 'positive', text: `${wellStaffed.map(r => r.role).join(', ')} fully staffed with strong coverage`, link: '/workforce' });
    }
  ...
  const rosterMembers = useMemo(() => {
    const rosterEmps = employees.filter(e => roster.includes(e.employee_id)).map(e => {
      ...
      return {
        name: e.name,
        role: e.role,
        source: isUpskilling ? 'Upskilled' : 'Internal',
        status: isUpskilling ? 'Training' : 'Active',
        score: score.total,
        risk: e.flight_risk?.toLowerCase() === 'high',
      };
    });

    const extMembers = externalCandidates
      .filter(c => shortlistedCandidates.includes(c.id))
      .map(c => ({
        name: c.name,
        role: c.current_role,
        source: 'External',
        status: c.notice_period_weeks > 0 ? 'Notice Period' : 'Active',
  ...
  if (!scenario) {
    return (
      <div>
        <PageHeader title="Team Efficiency" />
            ...
            <tbody>
              {rosterMembers.map(m => (
                <tr key={m.name} className="border-b border-border hover:bg-secondary/30">
                  ...
                  <td className="p-3"><Badge variant={sourceColors[m.source] || 'badge-blue'}>{m.source}</Badge></td>
                  <td className="p-3 text-muted-foreground">{m.status}</td>
                  <td className="p-3"><span className={`font-bold ${getScoreColor(m.score)}`}>{m.score}</span></td>
